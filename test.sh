...
TODAY="$(date +%F)"
PLANNING_DATE="${PLANNING_DATE:-$(date -d '+2 day' +%F 2>/dev/null || date -v+2d +%F)}"
...
show_json /tmp/onenext-health.json
...
show_json /tmp/onenext-empty-compile.json
...
assert_json /tmp/onenext-compile.json '.timeline[0].duration and (.focus.title == .timeline[0].title) and (.focus.duration == .timeline[0].duration)'
show_json /tmp/onenext-compile.json

echo "4. save plan"
request POST "/api/v1/plans" "{\"raw_input\":\"Tomorrow I have a quiz and need to reply to Ali. Also next Tuesday at 15:00 I have a meeting.\",\"planning_date\":\"$PLANNING_DATE\"}" > /tmp/onenext-saved-plan.json
...
assert_json_arg /tmp/onenext-saved-plan.json planning_date "$PLANNING_DATE" '.planning_date == $planning_date'
assert_json /tmp/onenext-saved-plan.json '.compiled_plan.timeline[0].duration and (.compiled_plan.focus.title == .compiled_plan.timeline[0].title) and (.compiled_plan.focus.duration == .compiled_plan.timeline[0].duration)'
show_json /tmp/onenext-saved-plan.json
...
show_json /tmp/onenext-plans.json
...
assert_json_arg /tmp/onenext-plan.json planning_date "$PLANNING_DATE" '.planning_date == $planning_date'
show_json /tmp/onenext-plan.json

echo "7. calendar returns derived events"
request GET "/api/v1/calendar?start=$PLANNING_DATE&end=$PLANNING_DATE" > /tmp/onenext-calendar-today.json
assert_json_arg /tmp/onenext-calendar-today.json plan_id "$plan_id" '.events | map(select(.plan_id == $plan_id)) | length > 0'
show_json /tmp/onenext-calendar-today.json
...
if [ "$future_date" = "null" ] || [ -z "$future_date" ]; then
  echo "No AI dated calendar events returned in this response; skipping future calendar persistence assertion."
else
  ...
  show_json /tmp/onenext-calendar-future.json
fi
